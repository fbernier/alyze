...
/// the sentence DFA actually has to look at. A byte stops the scan if it's non-ASCII (>= 0x80) or
/// one of the sentence-relevant ASCII bytes `\n` `\r` `!` `.` `?` (ParaSep / STerm / ATerm);
/// everything else is plain interior text we can skip over.
///
/// Only safe to call from the `Any`/`Upper`/`Lower` states, where every other ASCII byte is a
/// NoBreak that keeps us in those same states (see those rows in `transitions.rs` — if they
/// change, revisit `is_sentence_stop_byte`). Stopping early is harmless since the DFA just resumes
/// where we left off, which is why we can fold the rare `\x0B`/`\x0C` controls into the `\n..=\r`
/// test for free.
#[inline]
...
mod tests {
    ...
    use crate::uax29::test_helpers::{test_against_uax29_break_tests, xorshift64};
